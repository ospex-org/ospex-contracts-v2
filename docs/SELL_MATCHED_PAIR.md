...
The full lifecycle of a `SaleListing` in `SecondaryMarketModule`: list → wait/update/cancel/buy → claim proceeds. Sellers list one side of a matched pair with a price and quantity; buyers commit to the listing's current state via a hash to prevent front-running. Partial buys scale price and profit proportionally; payment flows through the contract and sellers claim accumulated USDC separately. Transferred positions are flagged with `acquiredViaSecondaryMarket = true` and are permanently ineligible for leaderboards.

See [ARCHITECTURE.md](./ARCHITECTURE.md#secondarymarketmodulesol) for the function reference and [DESIGN_DECISIONS.md](./DESIGN_DECISIONS.md#secondary-market-positions-permanently-ineligible-for-leaderboards) for the leaderboard-ineligibility rationale.
