...
|------|-------|------|---------|-------|
| Deploy / register the CRE workflow | N/A | `ospex-cre` workflow on Amoy | `ospex-cre` workflow, owned by the governance adapter | Point the workflow config (`receiverAddress` + `eventAddress`) at this `CreOracleReceiver` |
| Point the workflow at the receiver | N/A | Direct workflow update | Timelocked `linkSelfAsOwner` + `updateWorkflow` via the governance adapter | The receiver's `workflowOwner` must match the registered workflow owner |
