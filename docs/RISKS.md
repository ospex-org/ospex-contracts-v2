...
- A compromise of the 2-of-3 CRE governance Safe could, after the 7-day timelock delay, push a malicious workflow update or pause the workflow (see §6, Key Management).
...
- **Timelock-governed workflow**: The CRE workflow is owned directly by an `OspexCreTimelock` per-action timelock (a close port of Chainlink's audited `Timelock.sol`, with a 2-of-3 Safe holding proposer/executor/canceller and a 7-day mainnet delay on workflow code updates), so any change to the verification/scoring logic is observable on-chain before it can take effect.
...
> **R4 (Chainlink Functions) — SUPERSEDED by the R5 Chainlink CRE oracle migration.** See CreOracleReceiver / the cre-oracle skill. The R4 "approved signer key" trust dependency below no longer exists — R5 has no approved signer and no on-chain script approvals. See the **CRE workflow-owner / governance Safe compromise** risk that replaces it.

- **CRE workflow-owner / governance Safe compromise** (replaces the R4 approved-signer risk): The CRE workflow is owned directly by an `OspexCreTimelock` per-action timelock (a close port of Chainlink's audited `Timelock.sol`), with proposer/executor/canceller held by a 2-of-3 Safe. A compromise of the 2-of-3 governance **Safe** (which holds proposer/executor/canceller) could, after the 7-day timelock delay, push a malicious workflow update or pause the workflow — a single-signer compromise is insufficient. It **cannot pause the oracle quickly** (pause is reachable but inherits the 7-day delay + Safe gate — only Vault secret allowlist requests have the 1-second fast lane) and **cannot touch protocol funds** (settlement is immutable; PositionModule has zero admin functions). *Mitigations:* the 2-of-3 Safe means no single signer can schedule or execute an operation; monitor scheduled timelock operations so any malicious operation is observable on-chain for the full 7-day window before it can take effect.
...
- **Timelock-governed oracle workflow.** The off-chain CRE workflow is owned directly by an `OspexCreTimelock` per-action timelock (a close port of Chainlink's audited `Timelock.sol`), with proposer/executor/canceller held by a 2-of-3 Safe. Pause, code-update, delete, and other lifecycle ops are reachable but timelocked 7 days and Safe-gated; only Vault secret allowlist requests (every `cre secrets` op) have a 1-second fast lane.
