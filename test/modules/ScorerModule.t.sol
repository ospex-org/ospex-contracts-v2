...
contract ScorerModuleTest is Test {
    ...
    function setUp() public {
        ...
        treasuryModule = new TreasuryModule(address(core), address(token), address(0x2), 1_000_000, 500_000, 500_000);
        ...
        addrs[9] = address(moneyline);
        types[10] = core.SPREAD_SCORER_MODULE();
        addrs[10] = address(spread);
        types[11] = core.TOTAL_SCORER_MODULE();
        addrs[11] = address(total);
    ...
    function testMoneyline_AwayWins() public {
        ...
        uint256 speculationId =
            speculationModule.createSpeculation(contestId, address(moneyline), 0, address(this), address(this));
        ...
        assertEq(uint256(s.winSide), uint256(WinSide.Away));
    }
    ...
    function testMoneyline_HomeWins() public {
        ...
        speculationModule.settleSpeculation(speculationId);
        Speculation memory s = speculationModule.getSpeculation(speculationId);
        assertEq(uint256(s.winSide), uint256(WinSide.Home));
    }
    ...
    function testMoneyline_Push() public {
        ...
        speculationModule.settleSpeculation(speculationId);
        Speculation memory s = speculationModule.getSpeculation(speculationId);
        assertEq(uint256(s.winSide), uint256(WinSide.Push));
    }
    ...
    function testMoneyline_Revert_NotSpeculationModule() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                MoneylineScorerModule.MoneylineScorerModule__NotSpeculationModule.selector, notSpeculationModule
            )
        );
    ...
    function testMoneyline_Revert_ModuleNotSet() public {
        ...
        types[0] = newCore.CONTEST_MODULE();
        addrs[0] = address(mockContest);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                MoneylineScorerModule.MoneylineScorerModule__ModuleNotSet.selector, keccak256("SPECULATION_MODULE")
            )
        );
    ...
    function testSpread_AwayCovers_PositiveSpread() public {
        ...
        uint256 speculationId =
            speculationModule.createSpeculation(contestId, address(spread), spreadNum, address(this), address(this));
        _finalizeContest(contestId);
        vm.warp(block.timestamp + 2);
        speculationModule.settleSpeculation(speculationId);
        Speculation memory s = speculationModule.getSpeculation(speculationId);
    ...
    function testSpread_Revert_NotSpeculationModule() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                SpreadScorerModule.SpreadScorerModule__NotSpeculationModule.selector, notSpeculationModule
            )
        );
        vm.prank(notSpeculationModule);
    ...
    function testTotal_Over() public {
        ...
        uint256 speculationId =
            speculationModule.createSpeculation(contestId, address(total), totalNum, address(this), address(this));
        _finalizeContest(contestId);
        vm.warp(block.timestamp + 2);
        speculationModule.settleSpeculation(speculationId);
        Speculation memory s = speculationModule.getSpeculation(speculationId);
        assertEq(uint256(s.winSide), uint256(WinSide.Over));
    }
    ...
    function testTotal_Under() public {
        ...
        speculationModule.settleSpeculation(speculationId);
        Speculation memory s = speculationModule.getSpeculation(speculationId);
        assertEq(uint256(s.winSide), uint256(WinSide.Under));
    }
    ...
    function testTotal_Revert_NotSpeculationModule() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                TotalScorerModule.TotalScorerModule__NotSpeculationModule.selector, notSpeculationModule
            )
        );
        vm.prank(notSpeculationModule);
    ...
    // --- Helpers ---
    function _contest(uint32 away, uint32 home, ContestStatus status) internal pure returns (Contest memory) {
        return Contest({
            awayScore: away,
            homeScore: home,
