...
contract SpeculationModuleTest is Test {
    ...
    function setUp() public {
        ...
        treasuryModule =
            new TreasuryModule(address(core), address(mockToken), address(0x2), 1_000_000, 500_000, 500_000);
        ...
        types[2] = core.POSITION_MODULE(); // test contract acts as POSITION_MODULE
    ...
    function testCreateSpeculation_Success() public {
        ...
        uint256 id = speculationModule.createSpeculation(1, moneylineScorerAddr, 0, address(this), speculationCreator);
        ...
        assertEq(uint256(s.speculationStatus), uint256(SpeculationStatus.Open));
        assertEq(uint256(s.winSide), uint256(WinSide.TBD));
    }
    ...
    function testSettleSpeculation_Success() public {
        ...
        Speculation memory s = speculationModule.getSpeculation(id);
        assertEq(uint256(s.speculationStatus), uint256(SpeculationStatus.Closed));
    }
    ...
    function testSettleSpeculation_AutoVoidsAfterCooldown() public {
        ...
        speculationModule.settleSpeculation(id);
        Speculation memory s = speculationModule.getSpeculation(id);
        assertEq(uint256(s.winSide), uint256(WinSide.Void));
        assertEq(uint256(s.speculationStatus), uint256(SpeculationStatus.Closed));
    }
    ...
    function testSettleSpeculation_RevertsIfNotStarted() public {
        ...
        vm.expectRevert(SpeculationModule.SpeculationModule__InvalidStartTime.selector);
        speculationModule.settleSpeculation(id);
    }
    ...
    function testSettleSpeculation_RevertsIfAlreadySettled() public {
        ...
        vm.warp(block.timestamp + 1 hours);
        speculationModule.settleSpeculation(id);
        vm.expectRevert(SpeculationModule.SpeculationModule__AlreadySettled.selector);
        speculationModule.settleSpeculation(id);
    }
    ...
    function testCreateSpeculation_RevertsWhenCalledByNonPositionModule() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(SpeculationModule.SpeculationModule__NotAuthorized.selector, nonPositionModule)
        );
        vm.prank(nonPositionModule);
        speculationModule.createSpeculation(1, moneylineScorerAddr, 0, address(0x123), speculationCreator);
    }
    ...
    function testCreateSpeculation_SuccessWhenCalledByPositionModule() public {
        ...
        uint256 id = speculationModule.createSpeculation(1, moneylineScorerAddr, 0, maker, takerAddr);
    ...
    function testCreateSpeculation_RevertsIfScorerNotApproved() public {
        ...
        vm.expectRevert(SpeculationModule.SpeculationModule__ScorerNotApproved.selector);
        speculationModule.createSpeculation(1, unapprovedScorer, 42, address(this), speculationCreator);
    }

    function testCreateSpeculation_SucceedsWithApprovedScorer() public {
        // All three registered scorer modules should work
        uint256 id = speculationModule.createSpeculation(1, spreadScorerAddr, -35, address(this), speculationCreator);
        Speculation memory s = speculationModule.getSpeculation(id);
    ...
    function testCreateSpeculation_RevertsOnScoredContest() public {
        ...
        speculationModule.createSpeculation(1, moneylineScorerAddr, 0, address(this), speculationCreator);
    }
    ...
    function testSettleSpeculation_ScoredContestSettlesByScorerAfterCooldown() public {
        ...
        Speculation memory s = speculationModule.getSpeculation(id);
        assertEq(uint256(s.speculationStatus), uint256(SpeculationStatus.Closed));
        assertEq(uint256(s.winSide), uint256(WinSide.Away), "should settle by scorer, not void");
    }
    ...
    function testSettleSpeculation_UnresolvedContestStillVoidsAfterCooldown() public {
        ...
        Speculation memory s = speculationModule.getSpeculation(id);
        assertEq(uint256(s.winSide), uint256(WinSide.Void), "unresolved should void");
    }
    ...
    function testSettleSpeculation_RevertsIfContestNotFinalized() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(SpeculationModule.SpeculationModule__ContestNotFinalized.selector, 1));
        speculationModule.settleSpeculation(id);
    }
    ...
    function testCreateSpeculation_RevertsIfMoneylineScorerWithNonZeroLineTicks() public {
        ...
        speculationModule.createSpeculation(1, moneylineScorerAddr, 5, address(this), speculationCreator);
    }
    ...
    function testCreateSpeculation_RevertsIfTotalScorerWithNegativeLineTicks() public {
        vm.expectRevert(SpeculationModule.SpeculationModule__InvalidLineTicks.selector);
        speculationModule.createSpeculation(1, totalScorerAddr, -100, address(this), speculationCreator);
    }

    function testCreateSpeculation_SucceedsIfTotalScorerWithPositiveLineTicks() public {
        uint256 id = speculationModule.createSpeculation(1, totalScorerAddr, 2250, address(this), speculationCreator);
        Speculation memory s = speculationModule.getSpeculation(id);
    ...
    function testCreateSpeculation_RevertsIfLineTicksOutOfRange() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(SpeculationModule.SpeculationModule__LineTicksOutOfRange.selector, badLine)
        );
        speculationModule.createSpeculation(1, spreadScorerAddr, badLine, address(this), speculationCreator);

        // No speculation recorded at the choke point.
        assertEq(speculationModule.getSpeculationId(1, spreadScorerAddr, badLine), 0, "no speculation created");
    }
    ...
    function testCreateSpeculation_SucceedsAtLineTicksBoundary() public {
        ...
        uint256 id = speculationModule.createSpeculation(1, spreadScorerAddr, max, address(this), speculationCreator);
        assertGt(id, 0, "boundary line accepted");
        assertEq(speculationModule.getSpeculationId(1, spreadScorerAddr, max), id, "speculation stored at boundary");
        Speculation memory s = speculationModule.getSpeculation(id);
    ...
    function testCreateSpeculation_RevertsOnUnverifiedContest() public {
        Contest memory unverified = Contest({
            ...
            contestStatus: ContestStatus.Unverified,
            contestCreator: address(this),
            rundownId: "rd",
            sportspageId: "",
            jsonoddsId: ""
        ...
        vm.expectRevert(SpeculationModule.SpeculationModule__InvalidContestStatus.selector);
        speculationModule.createSpeculation(99, moneylineScorerAddr, 0, address(this), speculationCreator);
    }
    ...
    function testSettleSpeculation_VoidLocksContest() public {
        ...
        assertEq(uint256(contest.contestStatus), uint256(ContestStatus.Voided));
    }

    function testSettleSpeculation_SiblingSpeculationsVoidAfterContestVoided() public {
        // Two speculations on the same contest
        uint256 idA = speculationModule.createSpeculation(1, moneylineScorerAddr, 0, address(this), speculationCreator);
        uint256 idB = speculationModule.createSpeculation(1, spreadScorerAddr, -35, address(this), speculationCreator);
        ...
        assertEq(uint256(sA.winSide), uint256(WinSide.Void), "speculation A should be void");
        assertEq(uint256(sB.winSide), uint256(WinSide.Void), "speculation B should be void");
        assertEq(uint256(sA.speculationStatus), uint256(SpeculationStatus.Closed));
        assertEq(uint256(sB.speculationStatus), uint256(SpeculationStatus.Closed));
    }
    ...
    function testSettleSpeculation_NoMixedOutcomesOnSameContest() public {
        ...
        uint256 idA = speculationModule.createSpeculation(1, moneylineScorerAddr, 0, address(this), speculationCreator);
        uint256 idB = speculationModule.createSpeculation(1, spreadScorerAddr, -35, address(this), speculationCreator);
        uint256 idC = speculationModule.createSpeculation(1, totalScorerAddr, 2250, address(this), speculationCreator);
        ...
        assertEq(uint256(sA.winSide), uint256(WinSide.Void), "A should be void");
        assertEq(uint256(sB.winSide), uint256(WinSide.Void), "B should be void");
        assertEq(uint256(sC.winSide), uint256(WinSide.Void), "C should be void");
    }
    ...
    function testSettleSpeculation_VoidBlocksLateScoring() public {
        ...
        Contest memory contest = mockContestModule.getContest(1);
        assertEq(
            uint256(contest.contestStatus),
            uint256(ContestStatus.Voided),
    ...
    function testSettleSpeculation_SecondVoidDoesNotCallVoidContestAgain() public {
        ...
        // Settle A — voids contest
        speculationModule.settleSpeculation(idA);
        assertEq(uint256(mockContestModule.getContest(1).contestStatus), uint256(ContestStatus.Voided));

        // Settle B — contest already Voided, should still succeed (skips inner voidContest call)
        speculationModule.settleSpeculation(idB);
        Speculation memory sB = speculationModule.getSpeculation(idB);
        assertEq(uint256(sB.winSide), uint256(WinSide.Void));
    }
}
