...
- **Chainlink/Oracle Modularity**: Oracle code can be modularized or relocated as needed for architecture optimization. Internal helper contracts for oracle logic are acceptable, though preserving existing oracle code functionality is preferred.
- **Bulk Operations**: Can be organized based on architectural best practices - no strong preference for location as long as it's logical
- **Types and Structs**: Prefer keeping shared types in a single OspexTypes.sol unless there's a compelling reason for further separation
- **Secondary Market**: Should remain focused on simple P2P transfers, with the ability to support additional markets through mapping updates (should be role-protected)
- **Access Control**: No strong preference - place access control where it makes most logical sense
- **Gas Optimization**: Use best judgment to optimize where sensible while maintaining code clarity
- **Interfaces**: No strong preference on granularity - design interface structure based on what makes most sense for the architecture
