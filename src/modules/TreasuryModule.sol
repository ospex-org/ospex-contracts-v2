...
contract TreasuryModule is ITreasuryModule {
    ...
    bytes32 public constant EVENT_SPLIT_FEE_PROCESSED = keccak256("SPLIT_FEE_PROCESSED");
    bytes32 public constant EVENT_LEADERBOARD_FUNDED = keccak256("LEADERBOARD_FUNDED");
    bytes32 public constant EVENT_LEADERBOARD_ENTRY_FEE_PROCESSED = keccak256("LEADERBOARD_ENTRY_FEE_PROCESSED");
    bytes32 public constant EVENT_PRIZE_POOL_CLAIMED = keccak256("PRIZE_POOL_CLAIMED");
    ...
    event FeeProcessed(address indexed payer, FeeType indexed feeType, uint256 amount);
    ...
    event SplitFeeProcessed(
        address indexed payer1, address indexed payer2, FeeType indexed feeType, uint256 firstHalf, uint256 secondHalf
    );
    ...
    event LeaderboardFunded(uint256 indexed leaderboardId, address indexed funder, uint256 amount);
    ...
    event LeaderboardEntryFeeProcessed(address indexed payer, uint256 amount, uint256 indexed leaderboardId);
    ...
    event PrizePoolClaimed(uint256 indexed leaderboardId, address indexed to, uint256 amount);
    ...
    modifier validLeaderboard(uint256 leaderboardId) {
        Leaderboard memory lb =
            ILeaderboardModule(i_ospexCore.getModule(LEADERBOARD_MODULE)).getLeaderboard(leaderboardId);
    ...
    constructor(
        ...
        if (ospexCore_ == address(0) || token_ == address(0) || protocolReceiver_ == address(0)) revert TreasuryModule__InvalidReceiver();
    ...
    function processFee(address payer, FeeType feeType) external override onlyCore {
        ...
        i_ospexCore.emitCoreEvent(EVENT_FEE_PROCESSED, abi.encode(payer, feeType, amount));
    }

    /// @inheritdoc ITreasuryModule
    function processSplitFee(address payer1, address payer2, FeeType feeType) external override onlyCore {
        ...
        i_ospexCore.emitCoreEvent(EVENT_SPLIT_FEE_PROCESSED, abi.encode(payer1, payer2, feeType, firstHalf, secondHalf));
    }

    // ──────────────────────────── Leaderboard Funding ─────────────────

    /// @inheritdoc ITreasuryModule
    function fundLeaderboard(uint256 leaderboardId, uint256 amount) external validLeaderboard(leaderboardId) {
        ...
        i_ospexCore.emitCoreEvent(EVENT_LEADERBOARD_FUNDED, abi.encode(leaderboardId, msg.sender, amount));
    }
    ...
    function processLeaderboardEntryFee(address payer, uint256 amount, uint256 leaderboardId)
        ...
        override
        onlyCore
        validLeaderboard(leaderboardId)
    {
        i_token.safeTransferFrom(payer, address(this), amount);
        s_leaderboardPrizePools[leaderboardId] += amount;

        emit LeaderboardEntryFeeProcessed(payer, amount, leaderboardId);
        i_ospexCore.emitCoreEvent(EVENT_LEADERBOARD_ENTRY_FEE_PROCESSED, abi.encode(payer, amount, leaderboardId));
    }

    // ──────────────────────────── Prize Pool Disbursement ─────────────

    /// @inheritdoc ITreasuryModule
    function claimPrizePool(uint256 leaderboardId, address to, uint256 amount) external override onlyLeaderboardModule {
        ...
        i_ospexCore.emitCoreEvent(EVENT_PRIZE_POOL_CLAIMED, abi.encode(leaderboardId, to, amount));
    }

    // ──────────────────────────── View Functions ──────────────────────

    /// @inheritdoc ITreasuryModule
    function getFeeRate(FeeType feeType) external view override returns (uint256 rate) {
        return s_feeRates[feeType];
    }

    /// @inheritdoc ITreasuryModule
    function getPrizePool(uint256 leaderboardId) external view override returns (uint256 balance) {
