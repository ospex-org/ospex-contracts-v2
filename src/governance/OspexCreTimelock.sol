...
// =============================================================================================
//  OspexCreTimelock — provenance & licensing
//  Vendored from Chainlink Staking v0.2 `Timelock.sol` (MIT), as audited by Code4rena in Aug 2023
//  (repo `code-423n4/2023-08-chainlink`, `src/timelock/Timelock.sol`), which is itself a
//  per-(target,selector) delay extension of OpenZeppelin's `TimelockController` v4.7.0 (MIT).
//  Both upstreams are MIT-licensed, so this file is redistributed under MIT (retained above) even
//  though the rest of this repository is BUSL-1.1 — SPDX is per-file and the licenses do not
//  conflict. Original copyright (c) Chainlink Labs / OpenZeppelin; modifications (c) ospex.org.
//
//  OSPEX MODIFICATION — exactly TWO functional lines differ from upstream (search "[OSPEX]"): the
//  per-(target,selector) delay now DEFAULTS to the global `s_minDelay` when unset (a stored 0 means
//  "inherit") and MAY be set BELOW the global. Upstream used max(per-selector, global) and forbade
//  sub-global values — which would force the global to 0 and make every UNCONFIGURED action instant
//  (default-open). The Ospex variant is default-SLOW: a forgotten/unset selector inherits the long
//  default (locked, not open), with a single explicit fast carve-out (the CRE WorkflowRegistry's
//  `allowlistRequest` key op) so a leaked API key can be rotated without waiting the full code delay.
//  Mechanical port-only changes: renamed `Timelock`->`OspexCreTimelock`; pragma `0.8.19`->`^0.8.26`;
//  `AccessControlEnumerable` import path bumped to OZ 5.x `access/extensions`. All other logic
//  (schedule / execute / hashing / roles) is byte-for-byte upstream.
// =============================================================================================
 ...
 */
contract OspexCreTimelock is AccessControlEnumerable {
  ...
  function getMinDelay(address target, bytes4 selector) public view returns (uint256) {
    ...
    // [OSPEX] 0 = unset -> inherit the global default; otherwise use the exact value (may be < global).
    // Upstream: `return delay >= s_minDelay ? delay : s_minDelay;` (max(), forbids a sub-global carve-out).
    return delay == 0 ? s_minDelay : delay;
  }
  ...
  function _setDelay(address target, bytes4 selector, uint256 newDelay) internal {
    // [OSPEX] allow per-selector delays BELOW the global default; reserve 0 to mean "inherit default".
    // Upstream: `require(newDelay >= s_minDelay, 'Timelock: insufficient delay');`
    require(newDelay > 0, 'Timelock: use >0 (0 means inherit default)');
